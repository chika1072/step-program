...
"""
オイラーの素数生成式を用いて、nに対して、計算によって得られた値を返す
"""
...
class HashTable:
    ...
    def __init__(self):
        ...
        # hash conflicts.
        self.bucket_size = generate_size(8)   # 97
        self.buckets = [None] * self.bucket_size
        self.item_count = 0
        self.size_number = 8   # generate_sizeのnを記録する
    


    """
    要素数がテーブルサイズの70%を上回ったら、30%強になるようにテーブルサイズを調整
    要素数がテーブルサイズの30%を下回ったら、70%弱になるようにテーブルサイズを調整
    """
    def rehash(self):
        ...
        # self.bucket_size > 145でself.bucket_sizeが97未満になるのを防ぐ
    ...
    def delete(self, key):
        ...
        while item:
            if item.key == key:
                if prev_item: # もしprev_timeが存在するなら、prev_itemとitem.nextを連結する
